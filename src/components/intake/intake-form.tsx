...
const STEP_LABELS = ['Deceased', 'Contact', 'Preferences', 'Confirm', 'Sign'];
  ...
  return (
    <div className="px-6 pt-5 pb-4">
      ...
      <p className="text-xs text-slate-400 mt-2">
        Step {current} of {total} — <span className="font-medium text-slate-600">{STEP_LABELS[current - 1]}</span>
      </p>
    </div>
  );
}
...
function TokenStatusBadge({ state }: { state: TokenState }) {
  const config: Record<TokenState, { label: string; className: string }> = {
    ...
    intent_confirmed: { label: 'Ready to Sign',  className: 'bg-blue-50 text-blue-600' },
    signing:          { label: 'Signing...',      className: 'bg-blue-50 text-blue-600' },
...
export function IntakeForm({ tenantSlug }: IntakeFormProps) {
  ...
  async function handleSignAll() {
    const pending = signatureTokens.filter((t) => tokenStates[t.token] !== 'signed');
  ...
  if (tenantError) {
    return (
      <div className="text-center py-16 px-6 space-y-3">
        <p className="font-semibold text-slate-800">This intake form is unavailable.</p>
        <p className="text-sm text-slate-500">Please contact the funeral home directly for assistance.</p>
      </div>
    );
  }
  ...
  if (allSigned && familyAccessToken) {
    return (
      <div className="text-center py-14 px-6 space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-xl font-semibold text-slate-900">Thank you — {tenantName}</h2>
          <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed">
            Your documents have been received. Our team will be in touch shortly to discuss next steps.
          </p>
        </div>
        <div className="pt-2 space-y-3">
          <a
            href={`/family/${familyAccessToken}`}
            className="inline-flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-xl font-medium hover:bg-slate-800 transition-colors text-sm"
          >
  ...
  return (
    <div>
      {/* Dark header — tenant name */}
      <div className="bg-slate-900 px-6 py-5">
        <p className="text-xs font-semibold tracking-widest uppercase text-slate-400 mb-0.5">
          Arrangement Services
        </p>
        <p className="text-base font-semibold text-white leading-tight">{tenantName}</p>
      </div>
      ...
      <div className="px-6 pb-8">
        ...
        {step === 1 && (
          <form onSubmit={step1Form.handleSubmit((v) => { setStep1Data(v); setStep(2); })} className="space-y-5">
            <h2 className="text-xl font-semibold text-slate-900">About the Deceased</h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-slate-700">First Name <span className="text-destructive">*</span></Label>
              ...
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-slate-700">Last Name <span className="text-destructive">*</span></Label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-slate-700">Date of Death</Label>
                <Input type="date" {...step1Form.register('dateOfDeath')} className="w-full h-12" />
              </div>
            </div>

            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-slate-700">Service Type <span className="text-destructive">*</span></Label>
              <Select
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-slate-700">Place of Death</Label>
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              />
              <Label htmlFor="veteranStatus" className="text-sm font-normal cursor-pointer text-slate-700">
            ...
            <Button type="submit" className="w-full h-12 text-base bg-slate-900 hover:bg-slate-800 text-white mt-2">
              Continue
            </Button>
          </form>
        )}
        ...
        {step === 2 && (
          <form onSubmit={step2Form.handleSubmit((v) => { setStep2Data(v); setStep(3); })} className="space-y-5">
            <h2 className="text-xl font-semibold text-slate-900">Primary Contact</h2>
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-slate-700">Relationship <span className="text-destructive">*</span></Label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-slate-700">Phone <span className="text-destructive">*</span></Label>
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              />
              <Label htmlFor="financiallyResponsible" className="text-sm font-normal cursor-pointer text-slate-700">
            ...
            <div className="border-t border-slate-100 pt-5">
              {!hasSecondary ? (
                <button
                  ...
                  className="flex items-center gap-2 text-sm text-slate-600 font-medium hover:text-slate-900 transition-colors"
                >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-slate-700">Second Contact</h3>
                    <button type="button" onClick={() => step2Form.setValue('hasSecondary', false)} className="text-slate-400 hover:text-destructive transition-colors">
            ...
            <div className="border-t border-slate-100 pt-5 space-y-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Authorized Representative</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium text-slate-700">Full Name <span className="text-destructive">*</span></Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium text-slate-700">Your Relationship <span className="text-destructive">*</span></Label>
                  <Input
              ...
              <div className="flex items-start gap-3">
                <Checkbox
                  ...
                  className="mt-0.5"
                />
                <Label htmlFor="authorizedRep" className="text-sm font-normal cursor-pointer leading-relaxed text-slate-700">
            ...
            <div className="flex gap-3 mt-2">
              ...
              <Button type="submit" className="flex-1 h-12 text-base bg-slate-900 hover:bg-slate-800 text-white">Continue</Button>
            </div>
          </form>
        )}
        ...
        {step === 3 && (
          <form onSubmit={step3Form.handleSubmit((v) => { setStep3Data(v); setStep(4); })} className="space-y-5">
            <h2 className="text-xl font-semibold text-slate-900">Service Preferences</h2>
            <div className="space-y-1.5">
        ...
        {step === 4 && (
          <form onSubmit={step4Form.handleSubmit(handleStep4Submit)} className="space-y-6">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-600 space-y-2">
              <p className="font-semibold text-slate-800">Financial Responsibility Statement</p>
              <p>
            ...
            <div className="flex items-start gap-3">
              <Checkbox
                ...
                className="mt-0.5"
              />
              <Label htmlFor="financialAck" className="text-sm font-normal cursor-pointer leading-relaxed text-slate-700">
            ...
            <div className="flex gap-3 mt-2">
              ...
              <Button type="submit" className="flex-1 h-12 text-base bg-slate-900 hover:bg-slate-800 text-white" disabled={isSubmitting}>
        ...
        {step === 5 && !allSigned && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Authorization & Electronic Signatures</h2>
              <p className="text-sm text-slate-500 mt-1.5 leading-relaxed">
                Please review the following documents and provide your signature. By signing, you authorize{' '}
                <span className="font-medium text-slate-700">{tenantName}</span> to proceed with funeral services.
              </p>
            </div>

            <div className="space-y-3">
              {signatureTokens.map((t) => {
                const state = tokenStates[t.token] ?? 'pending';
                return (
                  <div
                    key={t.token}
                    className={cn(
                      'border-l-4 rounded-r-xl border rounded-xl pl-4 pr-4 py-4 space-y-2 transition-colors',
                      state === 'signed'
                        ? 'border-l-green-500 bg-green-50/40 border-green-100'
                        : state === 'error'
                          ? 'border-l-red-400 bg-red-50/40 border-red-100'
                          : 'border-l-slate-300 bg-slate-50/60 border-slate-100',
                    )}
                  >
            ...
            <div className="flex items-start gap-3">
              <Checkbox
                ...
                className="mt-0.5"
              />
              <Label htmlFor="intent" className="text-sm font-normal leading-relaxed cursor-pointer text-slate-700">
                I have read the above documents and intend to sign them electronically.
                {intentLoading && <span className="text-slate-400 ml-1">(confirming...)</span>}
                <span className="text-destructive"> *</span>
              </Label>
            </div>
            ...
            <div className="space-y-2">
              ...
              {!intentChecked && (
                <p className="text-xs text-slate-400">Confirm intent above to enable signature.</p>
              )}
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition-colors"
      >
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-slate-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-slate-700">State of Death</Label>
              <Input {...form.register('stateOfDeath')} className="w-full text-base h-12" placeholder="OH" maxLength={2} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-slate-700">State of Birth</Label>
          ...
          <div className="space-y-1.5">
            <Label className="text-sm font-medium text-slate-700">Faith Tradition</Label>
